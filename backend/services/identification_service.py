...
class IdentificationService:
    ...
    ):  # esta funcion retornara los datos mediante el esquema RegistrationResponse
        identification = await IdentificationRepository.get_by_code(
            session, code
        )  # aca simplemente trae el retorno del objeto que ya fue consultado en repository,

        if (
            identification
        ):  # en caso de si traer el objeto carga estos datos para enviarlos
